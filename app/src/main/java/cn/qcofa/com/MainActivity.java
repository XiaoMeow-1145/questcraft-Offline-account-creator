...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupClickListeners() {
        Button generateUuidBtn = findViewById(R.id.generateUuidBtn);
        generateUuidBtn.setOnClickListener(v -> generateUUID());
        ...
        Button importAccountBtn = findViewById(R.id.importAccountBtn);
        importAccountBtn.setOnClickListener(v -> showImportInstructions());
    }
    ...
    private void showImportInstructions() {
        // 检查是否已安装MT文件管理器
        boolean isMtInstalled = isAppInstalled("com.speedsoftware.rootexplorer") || 
                                isAppInstalled("com.sherif.mtmanager") || 
                                isAppInstalled("com.sherif.mt.filemanager");
        
        if (isMtInstalled) {
            // 如果已安装，则直接创建.sh文件并跳转到MT文件管理器
            createShellScriptAndLaunchMT();
        } else {
            // 如果未安装，则显示对话框提示下载
            showDownloadDialog();
        }
    ...
    private void showDownloadDialog() {
        ...
        builder.setTitle("导入账号");
        
        String message = "该功能可能需要在终端运行，你也可以使用Termux或者其他终端\n\n" +
                "点击下方按钮下载MT文件管理器";
        builder.setMessage(message);
        
        builder.setPositiveButton("下载MT文件管理器", (dialog, which) -> {
            openDownloadLink();
        });
    ...
    private void createShellScriptAndLaunchMT() {
        try {
            // 读取原始的ls.sh文件内容
            String scriptContent = "#!/bin/bash\n" +
                    "\n" +
                    "# 源目录\n" +
                    "SRC_DIR=\"/storage/emulated/0/Android/data/cn.qcofa.com/files/questcraft_accounts\"\n" +
                    "# 目标目录\n" +
                    "DST_DIR=\"/data/user/0/com.qcxr.qcxr/files/accounts\"\n" +
                    "\n" +
                    "echo \"==================== 导入账号 ====================\"\n" +
                    "if [ ! -d \"$SRC_DIR\" ]; then\n" +
                    ...
                    "\n" +
                    "# 只筛选 json 文件\n" +
                    "FILES=$(ls \"$SRC_DIR\"/*.json 2>/dev/null)\n" +
                    "if [ -z \"$FILES\" ]; then\n" +
                    "    echo \"目录中没有找到 .json 账号文件\"\n" +
                    "    exit 1\n" +
                    "fi\n" +
                    "\n" +
                    "echo \"找到可用账号文件：\"\n" +
                    ...
                    "fi\n" +
                    "\n" +
                    "echo \"正在导入账号：$(basename $SEL)\"\n" +
                    "\n" +
                    "# 利用 run-as 复制进去（免root）\n" +
                    "adb shell \"\n" +
                    "run-as com.qcxr.qcxr\n" +
                    "mkdir -p $DST_DIR\n" +
                    "cp '$SEL' $DST_DIR\n" +
                    "exit\n" +
                    "\"\n" +
                    "\n" +
                    "echo \"导入完成，路径：$DST_DIR\"\n";

            // 将文件保存到外部存储根目录
            java.io.File destFile = new java.io.File("/storage/emulated/0/", "ls.sh");
            
            // 写入文件
            java.io.FileWriter writer = new java.io.FileWriter(destFile);
            writer.write(scriptContent);
            writer.close();
            
            // 启动MT文件管理器并导航到.sh文件位置
            launchMTFileManager(destFile);
            
        } catch (Exception e) {
            Log.e(TAG, "创建脚本文件失败", e);
            Toast.makeText(this, "创建脚本文件失败: " + e.getMessage(), Toast.LENGTH_LONG).show();
        }
    }
    
    private void launchMTFileManager(java.io.File scriptFile) {
        try {
            // 尝试启动MT文件管理器并导航到脚本文件位置
            android.content.Intent intent = new android.content.Intent(android.content.Intent.ACTION_VIEW);
            
            // 获取文件URI
            android.net.Uri fileUri = android.net.Uri.fromFile(scriptFile);
            intent.setDataAndType(fileUri, "text/plain");
            
            // 尝试使用多个可能的MT文件管理器包名
            intent.setPackage("com.speedsoftware.rootexplorer"); // 旧版MT管理器
            if (!isIntentAvailable(intent)) {
                intent.setPackage("com.sherif.mtmanager"); // 新版MT管理器
                if (!isIntentAvailable(intent)) {
                    intent.setPackage("com.sherif.mt.filemanager"); // 另一个可能的包名
                    if (!isIntentAvailable(intent)) {
                        // 如果特定包名不可用，使用默认的查看意图
            ...
            Toast.makeText(this, "正在启动MT文件管理器...", Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
            Log.e(TAG, "启动MT文件管理器失败", e);
            // 如果启动失败，至少告知用户脚本已创建
            Toast.makeText(this, "脚本文件已创建，路径：" + scriptFile.getAbsolutePath(), Toast.LENGTH_LONG).show();
        }
    }
    
    private boolean isIntentAvailable(android.content.Intent intent) {
        android.content.pm.PackageManager packageManager = getPackageManager();
        java.util.List<android.content.pm.ResolveInfo> list = packageManager.queryIntentActivities(intent, android.content.pm.PackageManager.MATCH_DEFAULT_ONLY);
        return list.size() > 0;
    }
    ...
    private void openDownloadLink() {
        String downloadUrl = "https://pan.mt2.cn/apk/26020493";
        android.content.Intent intent = new android.content.Intent(android.content.Intent.ACTION_VIEW);
        intent.setData(android.net.Uri.parse(downloadUrl));
        try {
            startActivity(intent);
        } catch (android.content.ActivityNotFoundException e) {
            Toast.makeText(this, "无法打开浏览器，请手动访问下载链接", Toast.LENGTH_LONG).show();
        }
    }
